...
update() {
    ...
    # If current window is not in this desktop, no need to update.
     ...
     # [[ -z "$(bspc query -W -d focused | grep "$win_source" )" ]] && return
     # [[ -z "$(bspc query -W -d "$CUR_MON_DESK" | grep "$win_source" )" ]] && return
      # [ "$(bspc query -W -d "$CUR_MON_DESK" | grep "$win_source" )" == "" ] && return
      # if [[ "$(bspc query -W -d focused)" == "" ]]
        # then winName="" && return
      # fi


    if [[ "$always_tab" = false ]]; then
        for i in $(bspc query -W -d focused); do
