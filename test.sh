...
while read -r line ; do
    ...
    T*)
        ...
        while [ $# -gt 0 ] ; do
            ...
            if [ ! -z $winTitle ]; then
                case $item in
                    A*) title="${title}${pBGS1}%{F$pActive}$(printf %${pSLANT}s) ${winTitle} $(printf %${pSLANT}s)${pBG}";; #active window
                    X*) title="${title}${pTAB}%{F$pRest}$(printf %${pSLANT}s)${AC}${winfocus} ${winID}${AB} ${winTitle} ${AE}$(printf %${pSLANT}s)${pBG}";; #inactive window
                esac
            fi
            shift
        done
    ...
    W*)
        ...
        wm_infos="$wm_infos${pWSRight}${pBG} "
    ;;
    esac
